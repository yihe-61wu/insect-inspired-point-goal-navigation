...
### Model 2: MB as a high-level modulator for goal-directed locomotion
Here we consider a variation of the goal-directed locomotion (point goal navigation) task.
The original task concerns if and how a robot can navigation from the starting position to the goal position,
given the coordinates of two positions, in an unknown environment with potential obstacles.
We investigate, specifically, how the robot learns to avoid potential obstacles and optimise its route, 
using visual memory (with zero prior knowledge).
                                                                                                            
We consider this specific variation of the goal-directed locomotion task,
because such tasks can be solved by the combination of dead reckoning (path integration, i.e., PI)
and obstacle avoidance,
two abilities possessed by insects but not necessarily involving MB.
As with Model 1, we aim to harness the capability of rapid (visual) associative learning of a MB circuit.
Thus, we build the second model consisting of an MB circuit, a PI module and a tactile (force)-based collision escape module.

This model is relatively high-level,
because the MB does not generate a control command directly.
Instead, it uses visual memory to modulate the desired heading direction generated by the PI module.
Another distinction from Model 1 is that MB learning here is triggered only by collision escape events.
As the model starts with zero prior knowledge and the collision escape module is tactile-based, 
the model is expected to learn visual cues of obstacles, 
...
While we are still working on comprehensive simulation and analysis of Model 2 (and its variations),
initial results demonstrate that the model can optmise route by learning to visually avoid static and moving obstacles,
and, similar to Model 1, is robust against steering bias.
With the results from the comprehensive analysis, we will write another paper on Model 2 in the upcoming one to two months.
